...
def run_scann():
	...
	f = open(args.dataset+"_result.txt", "w")
	...
	for bc, sc in zip(build_config, search_config):
		...
		for arg in sc:
			...
			for split in range(args.num_split):
				...
				# ScaNN search
				def single_query(qid, query, base_idx):
					start = time.time()
					...
					# print("neighbors: ", local_neighbors.shape)
					# print("neighbors: ", type(local_distances))
					# neighbors[qid] = np.append(neighbors[qid], local_neighbors.view(1, args.topk)+base_idx, axis=0)
					# distances[qid] = np.append(distances[qid], local_distances.view(1, args.topk), axis=0)
					return (time.time() - start, (local_neighbors, local_distances))
				# ScaNN search
				print("Entering ScaNN searcher")
				local_results = [single_query(qid, q, base_idx) for qid, q in enumerate(queries)]
